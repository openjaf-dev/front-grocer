// This is a manifest file that'll be compiled into application.js, which will include all the files
// listed below.
//
// Any JavaScript/Coffee file within this directory, lib/assets/javascripts, vendor/assets/javascripts,
// or vendor/assets/javascripts of plugins, if any, can be referenced here using a relative path.
//
// It's not advisable to add code directly here, but if you do, it'll appear at the bottom of the
// compiled file.
//
// Read Sprockets README (https://github.com/sstephenson/sprockets#sprockets-directives) for details
// about supported directives.
//

//= require jquery
//= require jquery_ujs
//= require turbolinks
//= require chartkick

//= require jquery-2.1.0.min

//= require bootstrap
//= require modernizr
//= require bootstrap-tour.custom
//= require king-common
//= require deliswitch

//= require bootstrap-switch.min
//= require jquery.masked-input.min
//= require bootstrap-multiselect
//= require jquery-ui-1.10.4.custom.min
//= require jQAllRangeSliders-min
//= require bootstrap-colorpicker.min
//= require jquery.simplecolorpicker
//= require bootstrap.touchspin
//= require bootstrap-datepicker
//= require daterangepicker
//= require moment.min
//= require bootstrap-slider
//= require select2.min.js
//= require bootstrap-tagsinput.min
//= require king-elements

//= require stat/colorbrewer
//= require stat/d3.min

//= require stat/jquery.easypiechart.min
//= require raphael-2.1.0.min
//= require stat/flot/jquery.flot.min
//= require stat/flot/jquery.flot.resize.min
//= require stat/flot/jquery.flot.time.min
//= require stat/flot/jquery.flot.pie.min
//= require stat/flot/jquery.flot.orderBars
//= require stat/flot/jquery.flot.tooltip.min
//= require stat/flot/jquery.flot.selection.min

//= require jquery.sparkline.min
//= require datatable/jquery.dataTables.min
//= require datatable/jquery.dataTables.bootstrap

//= require jqgrid/jquery.jqGrid.min
//= require jqgrid/i18n/grid.locale-en
//= require jqgrid/jquery.jqGrid.fluid

//= require raphael-min
//= require jquery.mapael
//= require maps/world_countries
// require maps/usa_states
//= require maps/world_countries
//= require king-chart-stat
//= require king-table
//= require king-components
//= require jquery.mousewheel

//= require jquery-ui-1.10.4.custom.min
//= require fullcalendar.min
//= require jquery.simplecolorpicker

//= require wizard.min
//= require parsley.min

//= require dropzone.min

//= require jquery.mockjax.min
//= require moment.min
//= require bootstrap-datepicker
//= require bootstrap-editable.min
//= require typeahead
//= require typeaheadjs.1.5.1
//= require select2.min
//= require address.custom
//= require demo-mock

//= require summernote.min
//= require markdown/markdown
//= require markdown/to-markdown
//= require markdown/bootstrap-markdown

//= require jquery.gritter.min
//= require bootstrap-progressbar
//= require king-components
//= require king-form-layouts
//= require king-page
//= require parsley.min
//= require bootstrap-multiselect
//= require king-chart-stat


// require bootstrap-sprockets
// require_self

//= require jquery
//= require jquery_ujs
//= require_tree

